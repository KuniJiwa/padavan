...
Start_AD () {
    ...
    if [ ! -f "/tmp/sdnsadnew.conf" ]; then
        ...
        if [ -f "/tmp/sdnsadnew.conf" ]; then
            grep -wq "address=" /tmp/sdnsadnew.conf
            check=$?
            if [ $check -ne 0 ] ; then
    ...
    case $action in
        ...
        start)
            ...
            if [ "$sdns_redirect" = "1" ] ; then
                ...
                if [ "$sdnse_enable" = "1" ] ; then
                    logger -t "SmartDNS" "DNSmasq 上游（第二服务器）已指向 127.0.0.1:$sdnse_port"
                fi
            fi
            ;;
    esac
    ...
    case $action in
        ...
        start)
            if [ "$sdns_redirected" != "2" ] && [ "$sdns_redirect" = "2" ] ; then
                ...
                if [ "$sdnse_enable" = "1" ] ; then
                    logger -t "SmartDNS" "正在添加 iptables 重定向：53 → $IPS4:$sdnse_port（第二服务器）"
                fi
            fi
            ;;
        reset)
...
Start_smartdns () {
    ...
    if [ "$smartdns_process"x = x ] ; then
        if [ "$hosts_type" = "SmartDNS" ] ; then
            logger -t "SmartDNS" "进程启动失败，正在尝试移除广告配置后重试"
            logger -t "SmartDNS" "已移除广告过滤配置，准备二次启动"
            logger -t "SmartDNS" "如二次启动成功，请检查广告规则文件格式是否兼容"
    ...
    smartdns_process=$(pidof smartdns | awk '{ print $1 }')
    if [ "$smartdns_process"x = x ] ; then
        logger -t "SmartDNS" "二次启动仍然失败"
        logger -t "SmartDNS" "多次启动失败，已自动停用并恢复 DNSmasq"
        logger -t "SmartDNS" "已恢复 DNSmasq 为默认 DNS 解析服务"
        nvram set sdns_enable=0
        sdns_enable=0
        action="stop"
        Stop_smartdns
...
Stop_smartdns () {
    ...
    if [ -n "$smartdns_process" ]; then
        logger -t "SmartDNS" "正在终止进程 (PID: $smartdns_process)"
    fi
    Change_adbyby
    Change_dnsmasq
    Change_iptable
